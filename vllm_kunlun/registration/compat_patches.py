...
# --- vllm.lora.layers.fused_moe: decline LoRA on monolithic MoE ------------

# Set once a monolithic MoE layer has been left unwrapped, so the adapter guard
# knows expert LoRA cannot be honoured in this process.
_MONOLITHIC_MOE_LORA_DISABLED = False
_MONOLITHIC_MOE_LORA_WARNED = False

# Opt-in switch for the Kunlun-native expert LoRA path.
_EXPERT_LORA_ENV = "KUNLUN_ENABLE_EXPERT_LORA"
_FALSEY = ("", "0", "false", "no", "off")


def _kunlun_expert_lora_enabled() -> bool:
    """Return whether Kunlun's own expert LoRA path handles MoE layers."""
    return os.environ.get(_EXPERT_LORA_ENV, "0").strip().lower() not in _FALSEY


def _moe_lora_applied(module: ModuleType) -> bool:
    """Return whether the MoE LoRA wrappers are absent or already patched."""
    cls = getattr(module, "FusedMoEWithLoRA", None)
    return cls is None or getattr(module, "_kunlun_moe_lora_patched", False)


def _routed_experts_are_monolithic(source_layer: object) -> bool:
    """Return whether ``source_layer`` runs a fused, non-decomposable MoE."""
    routed_experts = getattr(source_layer, "routed_experts", None)
    quant_method = getattr(routed_experts, "quant_method", None)
    return bool(getattr(quant_method, "is_monolithic", False))


def _monolithic_aware_can_replace(lora_cls: type) -> classmethod:
    """Wrap ``can_replace_layer`` so monolithic MoE layers are declined."""
    original = lora_cls.can_replace_layer.__func__

    def can_replace_layer(
        cls, source_layer, lora_config, packed_modules_list, model_config=None
    ) -> bool:
        if not original(
            cls,
            ...
            return False
        if not _routed_experts_are_monolithic(source_layer):
            return True

        if _kunlun_expert_lora_enabled():
            return False

        global _MONOLITHIC_MOE_LORA_DISABLED, _MONOLITHIC_MOE_LORA_WARNED
        _MONOLITHIC_MOE_LORA_DISABLED = True
        if not _MONOLITHIC_MOE_LORA_WARNED:
            _MONOLITHIC_MOE_LORA_WARNED = True
            logging.getLogger("vllm_kunlun").warning(
                "[KunlunPlugin] fused MoE LoRA disabled: the Kunlun MoE kernel"
                " is monolithic, so %s cannot wrap the expert layers. LoRA on"
                " attention/MLP linear layers is unaffected; adapters that"
                " target experts will be rejected at load time.",
                cls.__name__,
            )
        return False

    return classmethod(can_replace_layer)


def _apply_moe_lora_patch(module: ModuleType) -> None:
    """Make both MoE LoRA wrappers refuse monolithic Kunlun MoE layers."""
    for name in ("FusedMoEWithLoRA", "FusedMoE3DWithLoRA"):
        lora_cls = getattr(module, name, None)
        if lora_cls is None:
            continue
        # Only patch a class that declares its own can_replace_layer;
        # FusedMoE3DWithLoRA inheriting it would otherwise be wrapped twice.
        if "can_replace_layer" not in vars(lora_cls):
            continue
        lora_cls.can_replace_layer = _monolithic_aware_can_replace(lora_cls)
    module._kunlun_moe_lora_patched = True


# --- vllm.lora.utils: register the Kunlun MoE LoRA layer -------------------


def _moe_lora_class_applied(module: ModuleType) -> bool:
    """Return whether the Kunlun MoE LoRA layer is registered (or not needed)."""
    if not hasattr(module, "_all_lora_classes"):
        return True
    return getattr(module, "_kunlun_moe_lora_class_registered", False)


def _apply_moe_lora_class(module: ModuleType) -> None:
    """Prepend ``KunlunFusedMoEWithLoRA`` to vLLM's LoRA layer table."""
    if not hasattr(module, "_all_lora_classes"):
        return
    if not _kunlun_expert_lora_enabled():
        module._kunlun_moe_lora_class_registered = True
        return
    from vllm_kunlun.lora.moe_lora_layer import KunlunFusedMoEWithLoRA

    module._all_lora_classes = (KunlunFusedMoEWithLoRA, *module._all_lora_classes)
    module._kunlun_moe_lora_class_registered = True
    logging.getLogger("vllm_kunlun").info(
        "[KunlunPlugin] registered KunlunFusedMoEWithLoRA for monolithic MoE"
    )


# --- vllm.lora.model_manager: fix the official PEFT 3D adapter conversion --
#
# Upstream's ``_convert_3d_to_2d_moe_lora`` assumes GPT-OSS-style in-major
# expert parameters and bails out when only one of gate_up / down was trained.
# See ``vllm_kunlun/lora/moe_lora_3d.py`` for the full reasoning.


def _moe_lora_3d_applied(module: ModuleType) -> bool:
    from vllm_kunlun.lora.moe_lora_3d import patch_applied

    return patch_applied(module)


def _apply_moe_lora_3d_patch(module: ModuleType) -> None:
    from vllm_kunlun.lora.moe_lora_3d import apply_patch

    apply_patch(module)


# --- vllm.lora.lora_model: reject silently-dropped expert LoRA -------------


def _expert_lora_guard_applied(module: ModuleType) -> bool:
    """Return whether ``LoRAModel`` is absent or already guarded."""
    cls = getattr(module, "LoRAModel", None)
    return cls is None or getattr(module, "_kunlun_expert_lora_guarded", False)


def _reject_expert_lora(lora_model: object, source: str) -> None:
    """Fail loudly when an adapter targets experts that cannot be applied.
    failure, so raise instead.
    """
    if _kunlun_expert_lora_enabled() or not _MONOLITHIC_MOE_LORA_DISABLED:
        return
    expert_modules = sorted(
        name for name in getattr(lora_model, "loras", {}) if ".experts" in name
    )
    if not expert_modules:
        return
    raise ValueError(
        f"LoRA adapter {source} contains expert weights "
        f"({len(expert_modules)} modules, e.g. {expert_modules[0]}), but the "
        "Kunlun fused MoE kernel is monolithic and cannot apply them. Loading "
        "would silently drop these weights. Re-train with target_modules "
        "restricted to attention/MLP linear layers, or merge the adapter into "
        "the base model and serve without --enable-lora."
    )


def _apply_expert_lora_guard(module: ModuleType) -> None:
    """Check every loaded ``LoRAModel`` for inapplicable expert weights."""
    lora_model_cls = getattr(module, "LoRAModel", None)
    if lora_model_cls is None:
        return
    original = lora_model_cls.from_local_checkpoint.__func__

    def from_local_checkpoint(cls, lora_dir, *args, **kwargs):
        lora_model = original(cls, lora_dir, *args, **kwargs)
        _reject_expert_lora(lora_model, lora_dir)
        return lora_model

    lora_model_cls.from_local_checkpoint = classmethod(from_local_checkpoint)
    module._kunlun_expert_lora_guarded = True
...
DEFAULT_HOOKS = (
    ...
    ("vllm.lora.layers.fused_moe", _moe_lora_applied, _apply_moe_lora_patch),
    ("vllm.lora.utils", _moe_lora_class_applied, _apply_moe_lora_class),
    ("vllm.lora.model_manager", _moe_lora_3d_applied, _apply_moe_lora_3d_patch),
    ("vllm.lora.lora_model", _expert_lora_guard_applied, _apply_expert_lora_guard),
) + tuple(
